...
| `WAPP_WORKER_INFERENCE_TIMEOUT_MS` | `45000` | Plazo **por defecto** de una inferencia — el que se aplica a la que llegue **sin** `timeout_ms` del Cloud. Su antiguo gemelo `WAPP_AGENT_INTENT_WAIT_MS` (el presupuesto del despachador) se retiró con el push en T1.6-5. ⚠️ **Esta tabla decía `15000` hasta el 2026-08-24 y era FALSO**: la Ola 1.6 lo subió a 45 s en el código. Y no es una errata inocua — el **umbral de lentitud del breaker se DERIVA de este número**, así que un `.env` que lo deje en 15000 no sólo acorta el plazo: hace que el breaker cuente como sanas peticiones que se comieron su presupuesto. **Confírmalo en el latido** (`inferencia_timeout_ms` en `cajero: arrancando`), nunca en esta tabla. |
...
| `WAPP_WORKER_NUM_THREAD` · `NUM_PREDICT` · `NUM_CTX` | `5` · `256` · `4096` | Opciones que se le pasan a Ollama por inferencia. Calibradas en la O0; no se tocan sin medición. ⚠️ Desde la Ola 1.7, `NUM_PREDICT` es el **default** del tope de salida: si el frame trae `max_output_tokens`, **manda el del Cloud**. Este número es el que se aplica cuando el Cloud calla, y es el mando con el que forzar el lado B del A/B del truncado desde el Edge. Efectivo en el latido: `num_predict_default`. |
| `WAPP_WORKER_KEEP_ALIVE_SECONDS` | `-1` | Segundos que Ollama mantiene el modelo cargado tras responder; viaja en **cada** petición, en el primer nivel de `/api/chat`. **`-1` = para siempre** (el default), **`0` = descárgalo en cuanto respondas**, positivo = segundos. 🔴 **Sin guardarraíl `<=0 ⇒ default`**, y es a propósito: los tres tramos significan cosas distintas y las tres son legítimas, así que el patrón habitual haría imposible pedir `0`. Existe porque cuando el runner muere por silencio **se lleva la caché de prefijos con él**, y el siguiente mensaje paga carga del modelo (**39 s medidos**) más el prefill en frío del prompt entero. Efectivo en el latido: `keep_alive_s`. |
| `WAPP_WORKER_PREFILL_FRIO_MS` · `PREFILL_CALIENTE_MS` | `5000` · `2000` | Los dos bordes con los que se clasifica el **calor del prefijo** para `inference_by_regime` del heartbeat: `> frio` ⇒ `frio`, `< caliente` ⇒ `caliente`, y la franja cerrada del medio ⇒ **`templado`**. Son **política de esta máquina** y no viajan en el contrato. 🔴 **El conteo de `templado` es la señal de que hay que moverlos**: están calibrados contra el VPS de UAT (agosto 2026), y en otro equipo la frontera cae en otro sitio. Guardarraíl: `<=0` cae al default, y una pareja **invertida o pegada** (`caliente >= frio`) cae **entera** al default con un `Warn` que dice qué se pidió y qué se aplicó — porque con esa pareja `templado` no puede darse nunca y el heartbeat publicaría una clave clavada a 0 sin explicar por qué. Efectivos en el latido: `prefill_caliente_ms` / `prefill_frio_ms`. |
