...
func TestAuthorize_ValidTokenAllows(t *testing.T) {
	...
	tok := mintES256(t, key, "es256-1", "t1", identityrbac.Grants{Allow: []string{"edge.*"}}, timeFuture())
...
func TestAuthorize_DefaultDeny(t *testing.T) {
	...
	tok := mintES256(t, key, "es256-1", "t1", identityrbac.Grants{Allow: []string{"edge.status.read"}}, timeFuture())
...
func TestAuthorize_DegradedReadAllowedWriteDenied(t *testing.T) {
	...
	expired := mintES256(t, key, "es256-1", "t1", identityrbac.Grants{Allow: []string{"edge.*"}}, timePast())
...
func TestAuthorize_DegradedOnlyHeldToken(t *testing.T) {
	relay := &fakeRelay{}
	m, _, key := setup(t, relay)
	held := mintES256(t, key, "es256-1", "t1", identityrbac.Grants{Allow: []string{"edge.*"}}, timePast())
	...
	otherExpired := mintES256(t, key, "es256-1", "t1", identityrbac.Grants{Allow: []string{"edge.*"}}, timePast())
...
func TestAuthorize_TenantMismatch(t *testing.T) {
	...
	tok := mintES256(t, key, "es256-1", "otro-tenant", identityrbac.Grants{Allow: []string{"edge.*"}}, timeFuture())
	relay.loginTokens = Tokens{AccessToken: tok, RefreshToken: "rt", ExpiresAt: timeFuture()}
	_, _ = m.Login(context.Background(), "op", "pw")
...
func TestProactiveRefresh_RefreshesNearExpiry(t *testing.T) {
	...
	near := mintES256(t, key, "es256-1", "t1", identityrbac.Grants{Allow: []string{"edge.*"}}, time.Now().Add(time.Minute))
	...
	fresh := mintES256(t, key, "es256-1", "t1", identityrbac.Grants{Allow: []string{"edge.*"}}, timeFuture())
