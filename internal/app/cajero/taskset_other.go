...
// afinidadOllama en NO-Linux: la comprobación de afinidad de CPU (T2.8) NO EXISTE fuera de Linux y
// aquí se dice explícitamente en vez de fingir un resultado.
//
...
// puede comprobar el aislamiento de CPU de Ollama.
func afinidadOllama(ctx context.Context, urlOllama string) (string, error) {
	if err := ctx.Err(); err != nil {
		return "", err
	}
	_ = urlOllama
	return "", fmt.Errorf("la afinidad de CPU sólo es observable en Linux (sched_getaffinity); este equipo es %s", runtime.GOOS)
}
