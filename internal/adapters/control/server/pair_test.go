...
// fakePairer es un doble del puerto Pairer: publica QRs en el sink y bloquea hasta que el test empuja
// el resultado terminal por release. Simula sellar una DEK ficticia (secretDEK) dentro de Run, fuera
// del contrato, para poder verificar la invariante DEK-fuera-del-/v1.
...
func (f *fakePairer) Run(ctx context.Context, sink app.QRSink) (string, error) {
	for _, qr := range f.qrs {
		_ = sink.ShowQR(qr)
	}
	select {
	case err := <-f.release:
		...
		return "fake-jid@s.whatsapp.net", nil
	case <-ctx.Done():
		return "", ctx.Err()
	}
}

var _ Pairer = (*fakePairer)(nil)

// startPairServer levanta el Server real sobre un Unix socket de prueba con el Pairer dado registrado,
// y devuelve un http.Client que marca por ese socket. Espejo de startServer (server_test.go) más
// RegisterPairing antes de Serve.
func startPairServer(t *testing.T, p Pairer) *http.Client {
	t.Helper()
