...
// fakeLister es un doble de SessionLister (el real es *sessionstore.Store). Permite testear los
// handlers sin BD ni WhatsApp.
type fakeLister struct {
	sessions []domain.Session
	err      error
}

func (f fakeLister) List(context.Context) ([]domain.Session, error) {
...
func TestSessions(t *testing.T) {
	...
	lister := fakeLister{sessions: []domain.Session{
		{JID: "111@s.whatsapp.net", State: domain.SessionStateActive, PairedAt: now, UpdatedAt: now},
		{JID: "222@s.whatsapp.net", State: domain.SessionStateLoggedOut},
	}}
	...
	if got.Sessions[0].JID != "111@s.whatsapp.net" || got.Sessions[0].State != "active" {
	...
	// El segundo tiene timestamps cero → se omiten (omitempty).
	if got.Sessions[1].PairedAt != "" || got.Sessions[1].UpdatedAt != "" {
		t.Errorf("timestamps cero deberían omitirse: %+v", got.Sessions[1])
	}
}
